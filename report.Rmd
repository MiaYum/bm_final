...
readRDS("math_table.rds") |>
  ...
  data.frame() |>
  knitr::kable(caption = "Math Model Coefficents") 
```
...
readRDS("reading_table.rds") |>
  lapply(function(x) if(is.numeric(x)) round(x, 3) else x) |>
  data.frame() |>
  knitr::kable(caption = "Reading Model Coefficents") 
```
...
readRDS("writing_table.rds") |>
  lapply(function(x) if(is.numeric(x)) round(x, 3) else x) |>
  data.frame() |>
  knitr::kable(caption = "Writing Model Coefficents") 
```