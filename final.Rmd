...
# 设置分割的比例
set.seed(123) # 设置一个随机种子以保证结果的可重复性
splitRatio <- 0.8
...
# Splitting the traindataset into independent variables (X) and dependent variables (Y)
...
model_math <- lm(Y_math_train ~ ., data = X_train)
model_reading <- lm(Y_reading_train ~ ., data = X_train)
model_writing <- lm(Y_writing_train ~ ., data = X_train)
...
model_math_interaction <- lm(Y_math_train ~  (.)^2, data = X_train)
...
model_reading_with_math <- lm(Y_reading_train ~ . + Y_math_train, data = XX_train)
