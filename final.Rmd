...
library(dplyr)
library(purrr)
data$NrSiblings <- as.character(data$NrSiblings)
# 列出所有字符变量的名字
char_vars <- names(data)[sapply(data, is.character)]

# 生成所有可能的变量对组合
var_combinations <- combn(char_vars, 2, simplify = FALSE)

# 对每一对变量进行Fisher精确检验
fisher_results <- map_dfr(var_combinations, ~{
  # 提取变量对
  var_pair <- .x

  # 从数据框中提取这两个变量
  tbl <- data %>% select(all_of(var_pair)) %>% na.omit()

  # 进行Fisher精确检验
 test_result <- fisher.test(tbl[[1]], tbl[[2]], simulate.p.value=TRUE)


# 返回变量对和对应的P值
  tibble(
    variable1 = var_pair[1],
    variable2 = var_pair[2],
...
lm(Y_math_train ~ ., data = X_train)
vif_values_math <- vif(lm(Y_math_train ~ ., data = X_train) )
...
#| message: false
par(mfrow=c(1,3))
plot(model_math_interaction, which = 4)
plot(model_reading_interaction, which = 4)
plot(model_writing_interaction, which = 4)
```


从图中发现181，642可能是异常或者高影响值，但是除去之后建立模型发现差别不大，并且我们单独查看这两行数据也没有发现异常信息，于是最终模型还是保留以上数据。
...
X_train_math_Out =X_train[-c(181,268),]
Y_math_train_Out=Y_math_train[-c(181,268)]
X_train_reading_Out =X_train[-c(181,268),]
Y_reading_train_Out=Y_reading_train[-c(181,268)]
X_train_writing_Out =X_train[-c(181,268),]
Y_writing_train_Out=Y_writing_train[-c(181,268)]
...
#| message: false
# 计算方差膨胀因子（GVIF）
vif_values_math <- vif(model_math_interaction , type = 'predictor')
print(vif_values_math)
vif_values_writing <- vif(model_writing_interaction, type = 'predictor')
print(vif_values_writing)
vif_values_reading <- vif(model_reading_interaction, type = 'predictor')
...
math_model_cv <- train( Y_math_train ~ Gender + EthnicGroup + ParentEduc + 
    LunchType + TestPrep + ParentMaritalStatus + PracticeSport + 
    IsFirstChild + NrSiblings + TransportMeans + WklyStudyHours + 
    Gender:LunchType + Gender:PracticeSport + EthnicGroup:ParentEduc + 
    EthnicGroup:IsFirstChild + ParentEduc:TestPrep + ParentEduc:ParentMaritalStatus + 
    ParentEduc:PracticeSport + ParentEduc:IsFirstChild + LunchType:PracticeSport + 
    LunchType:TransportMeans + TestPrep:WklyStudyHours + ParentMaritalStatus:PracticeSport + 
    ParentMaritalStatus:IsFirstChild + ParentMaritalStatus:TransportMeans + 
    PracticeSport:WklyStudyHours + IsFirstChild:NrSiblings + 
...
cv_df_math =
  cv_df_math |> 
  mutate(
    model  = map(train, ~lm( Y_math_train ~ Gender + EthnicGroup + ParentEduc + 
    LunchType + TestPrep + ParentMaritalStatus + PracticeSport + 
    IsFirstChild + NrSiblings + TransportMeans + WklyStudyHours + 
    Gender:LunchType + Gender:PracticeSport + EthnicGroup:ParentEduc + 
    EthnicGroup:IsFirstChild + ParentEduc:TestPrep + ParentEduc:ParentMaritalStatus + 
...
#| echo: false
#| message: false
final_math=model_math_interaction
final_reading=model_reading_interaction
final_writing=model_writing_interaction
```
